...
def service_status(service: Service) -> Optional[bool]:
  ...
  for task in service.tasks():
    ...
    if task_state == 'complete':
      complete_state = not (complete_state is False)
      continue
...
class SwarmExecutor(BaseExecutor, LoggingMixin):
  ...
  # TODO: Use executor config?  Meh, this actually seems easier.
  WORK_BATCH_SIZE = int(os.environ.get('SWARM_EXECUTOR_BATCH_SIZE') or 10)
  ...
  def work_to_converge(self):
    while True:
      services = self.executor_services(filter_label=self.scheduler_label(self.job_id))
      self.log.debug('Found %s services belonging to this scheduler', len(services))
  ...
  def sync(self) -> None:
    ...
    for unit in itertools.islice(self.work_to_converge(), self.WORK_BATCH_SIZE):
      try:
        unit()
      except:
  ...
  def scheduler_label(self, job_id=None):
    return {'airflow-scheduler-id': job_id or self.job_id}

  def executor_label(self):
    return {'airflow-executor': 'SwarmExecutor1'}

  def end(self) -> None:
    """Synchronously complete all work"""
    # TODO: flush the whole work queue.
    try:
