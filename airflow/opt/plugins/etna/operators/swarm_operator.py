...
from typing import Optional, List, Dict, Callable, Any, Union, Iterator
...
from docker.types import ConfigReference, Mount, SecretReference, ServiceMode
...
class SwarmServiceDefinition:
  ...
  mem_limit: int
  networks: List[Union[str, types.NetworkAttachmentConfig]]
  placement: Union[types.Placement, List[types.Placement]]
  mode: ServiceMode
...
def create_service_definition(
        data: Dict,
        include_external_networks: bool = False,
...
def swarm_cleanup(cli: APIClient):
  ...
  for service in cli.services(filters={'label': join_labels(DockerSwarmOperator.service_labeling())}):
    ...
    if task_status['State'] in ['complete', 'failed', 'shutdown', 'rejected', 'orphaned', 'remove']:
      ...
      t: datetime = datetime.strptime(timestamp, datetime.isoformat(timestamp))
...
class DockerSwarmOperator(BaseOperator):
  ...
  def execute(self, context) -> None:
    ...
    return self._run_service(create_service_definition(
      find_service(self.cli, self.source_service),
      include_external_networks=self.include_external_networks
    ))
  ...
  def _find_or_create_service(self, service_definition: SwarmServiceDefinition) -> Service:
  ...
  def _cleanup_service(self):
    ...
    for config_spec_json in configs_json:
      ...
      if all(config_labels.get(key) == v for key, v in self.shared_data_labeling().items()):
  ...
  def _consume_logs(self, service_definition: SwarmServiceDefinition) -> Any:
    log_chunk_iter = self.cli.service_logs(self.service['ID'], follow=True, stdout=True, stderr=True, is_tty=service_definition.tty)
...
def consume_lines(chunk_iter: Iterator[bytes], last_line_buffer: List[bytes]) -> Iterator[str]:
  buffer_ready = False
