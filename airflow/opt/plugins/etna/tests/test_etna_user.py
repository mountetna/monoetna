from etna.auth.etna_user import parse_permissions, permission_parser, EtnaUser, Permission
...
def test_parse_permissions():
  ...
  assert permissions == [
    ...
    Permission(project_name='coprojects_template', see_restricted=False, role='admin'),
    ...
    Permission(project_name='zach_test_project_one', see_restricted=False, role='admin'),
...
def test_parse_permission():
  permissions = take_left(permission_parser, eof_parser).run_parser('A:project1,project2')[0]
  assert permissions == [
  ...
  permissions = take_left(permission_parser, eof_parser).run_parser('v:project5,project1,project2')[0]
  assert permissions == [
