...
def sc_file_linkers(helpers: MetisEtlHelpers, listed_matches):
    helpers.link_matching_file(listed_matches, 'cite_antibody_key', re.compile(r'^ADT_keep_features\.list$'))
    helpers.link_matching_file(listed_matches, 'mux_index_key', re.compile(r'^IDX_map\.tsx$'))
    helpers.link_matching_file(listed_matches, 'processed_robject_rdata', re.compile(r'.*_scTransformed_processed\.RData$'))
    helpers.link_matching_file(listed_matches, 'processed_umap', re.compile(r'.*_umap\.pdf$'))
    helpers.link_matching_file(listed_matches, 'processing_pipeline_parameters', re.compile(r'^cutoffs\.yml$'))
    helpers.link_matching_file(listed_matches, 'filtered_counts_h5', re.compile(r'^filtered_feature_bc_matrix\.h5$'))
    helpers.link_matching_file(listed_matches, 'raw_counts_h5', re.compile(r'^raw_feature_bc_matrix\.h5$'))
    helpers.link_matching_file(listed_matches, 'tenx_aligned_bam', re.compile(r'^possorted_genome_bam\.bam$'))
    helpers.link_matching_file(listed_matches, 'tenx_aligned_bam_index', re.compile(r'^possorted_genome_bam\.bam\.bai$'))
    helpers.link_matching_file(listed_matches, 'tenx_cloupe_file', re.compile(r'^cloupe\.cloupe$'))
    helpers.link_matching_file(listed_matches, 'tenx_metrics_csv', re.compile(r'^metrics_summary\.csv$'))
    helpers.link_matching_file(listed_matches, 'tenx_molecule_info_h5', re.compile(r'^molecule_info\.h5$'))
    helpers.link_matching_file(listed_matches, 'tenx_web_summary', re.compile(r'^web_summary\.html$'))
...
def ipi_data_metis_etl(helpers: MetisEtlHelpers):
    with TaskGroup('sc_rna_seq_pool_raw'):
        matches = helpers.find_record_folders('sc_rna_seq_pool',
        ...
        helpers.link_matching_files(listed_matches, 'raw_fastq', file_regex=re.compile(r'.*\.fastq\.gz$'),
                                    dry_run=False)
    ...
    with TaskGroup('sc_rna_seq_raw'):
        matches = helpers.find_record_folders('sc_rna_seq',
    ...
    with TaskGroup('rna_seq_bulk'):
        matches = helpers.find_record_folders('-', re.compile(r'^bulkRNASeq/processed/[^/]*/results'))
        listed_matches = helpers.list_match_folders(matches)
