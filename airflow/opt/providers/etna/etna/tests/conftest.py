...
# When re-recording tests, you can use this callback in before_record_response
#   to avoid recording the task token generated by Janus, in the cassettes.
# Change the test decorator to:
# @pytest.mark.vcr(before_record_response=scrub_string('foo'), record_mode='once')
...
@pytest.fixture()
def reset_db(close_db_connections):
