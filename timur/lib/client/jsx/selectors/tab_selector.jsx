...
const filterAttributes = (attributes, types, exclude = false) =>
  Object.keys(attributes)
    .filter(
      (attribute_name) =>
        !attributes[attribute_name].hidden &&
        (!types || types.includes(attributes[attribute_name].attribute_type)
          ? !exclude
          : exclude)
    )
    .sort((a, b) => a.localeCompare(b));

const overviewItems = (attributes, others = []) =>
  filterAttributes(attributes, ['parent'])
    .concat(filterAttributes(attributes, ['link', 'collection', 'child']))
    .concat(
      filterAttributes(
        attributes,
        ['link', 'collection', 'child', 'parent', 'identifier'].concat(others),
        true
      )
    )
    .map(attributeItem);
...
const groupView = ({attributes}) => {
  let groups = Object.keys(attributes).reduce((groups, attribute_name) => {
    let {attribute_group = 'overview', attribute_type} =
  ...
  return Object.keys(groups)
    .sort((g1, g2) =>
      g1 == 'overview' ? -1 : g2 == 'overview' ? 1 : g1.localeCompare(g2)
    )
    .map((group) => ({
      name: group,
      panes: [
        {
          items:
            group == 'overview'
              ? overviewItems(groups[group])
              : filterAttributes(groups[group]).map(attributeItem)
        }
      ]
    }));
};
...
export const hasMagmaAttribute = (item) =>
  item.type == 'magma' ||
  (item.type == 'markdown' && item.attribute_name) ||
  (item.type == 'image' && item.attribute_name);

export const getAttributes = ({panes}) =>
  panes
    .map(({items}) =>
