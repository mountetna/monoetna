...
import MagmaLink from 'etna-js/components/magma_link';
    ...
    <div className='drop_down'>
      {Object.keys(matches).map((modelName) => (
        <div key={modelName}>
          ...
          <div className='list'>
            {matches[modelName].map((identifier) => (
              <div
                key={identifier}
...
class IdentifierSearch extends React.Component {
  ...
  findMatches() {
    ...
    let matches = Object.entries(identifiers).reduce(
  ...
  setMatch(match_string) {
    this.setState({match_string});
  }
    ...
    return (
      ...
      >
        <div className='search'>
          ...
          {matches && (
            <i
              className='dismiss fa fa-times'
              onClick={(e) => this.setMatch('')}
            />
          )}
        </div>
