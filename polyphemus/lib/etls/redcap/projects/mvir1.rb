...
define_model("Patient").class_eval do
  ...
  CONSENT = {
    ...
    "50" => "Initial Waiver",
    "0" => "No Samples/Data",
  } unless defined?(CONSENT)

  def patch(magma_record_name, record)
    record[:consent] = CONSENT[record[:consent]]
    record[:age] = [89, record[:age]].min if record[:age]
    record[:age_at_death] = [89, record[:age_at_death]].min if record[:age_at_death]
  end
end

define_model("Timepoint").class_eval do
  ...
  def offset_id(magma_record_name)
    magma_record_name.split(/-/)[0..1].join("-")
  end

  def patch(magma_record_name, record)
    record[:patient] = magma_record_name.split(/-/)[0..1].join("-")
    record[:day] = magma_record_name[/-D([0-9]+)$/, 1].to_i
  end
end
  ...
  def self.included(base)
    base.class_eval do
      ...
      def patient_id(magma_temp_id)
        "MVIR1-HS#{magma_temp_id.split(/-/)[1].to_i - 1000}"
      end
...
define_model("Symptom").class_eval do
  include PatientTable
end
...
define_model("AdmissionLab").class_eval do
  include PatientTable
end

define_model("ClinicalLab").class_eval do
  def offset_id(magma_temp_id)
    "MVIR1-HS#{magma_temp_id.split(/-/)[1].to_i - 1000}"
  end

  def patch(magma_temp_id, record)
    record_name, event_name = magma_temp_id.split(/-/)[1..2]

    day = event_name[/^D[0-9]+/]

    record[:timepoint] = day ? "MVIR1-HS#{record_name.to_i - 1000}-#{day}" : nil
  end
end
...
module CometAssay
  def self.included(base)
    base.class_eval do
      # We override these methods via the included hook,
      #   otherwise just including the CometAssay module
      #   means they will be overwritten by the default
      #   method definitions.
      def redcap_id(record_name, record)
        record_name.match(/MVIR1-HS(\d+)-(D\d+)/) do |m|
          patient_num, day = m[1..2]

          return [(patient_num.to_i + 1000).to_s, day + " (Arm 1: Inpatient Visits)"]
        end
      ...
      def offset_id(record_id)
        puts record_id
        record_id.split(/-/)[0..1].join("-")
      end
    end
  end
end

define_model("RnaSeq").class_eval do
  include CometAssay
end

define_model("ScRnaSeq").class_eval do
  include CometAssay
end

define_model("Immunoassay").class_eval do
  include CometAssay
end

define_model("Cytof").class_eval do
  include CometAssay
end
        ...
        scripts: [
          {
            attributes: {
              ...
              vasopressor_dose: "tx_pressor_dose",
              vasopressors: {
