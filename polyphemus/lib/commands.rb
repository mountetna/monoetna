...
class Polyphemus
  ...
  class LoadTable < Etna::Command
    include WithEtnaClientsByEnvironment
    include WithLogger
    usage 'load_table <env> <project_name> <model_name> <file_path> [--execute]'
    boolean_flags << '--execute'

    def execute(env, project_name, model_name, file_path, execute: false)
      @environ = environment(env)
      @project_name = project_name
      @model_name = model_name
