...
class SFTPMetisUploaderJob < Polyphemus::ETLJob
  ...
  def pre(context)
    ...
    context[:failed_files_path] = File.join(File.dirname(run["state"]["files_to_update_path"]), "metis_failed_files.csv")
  end
  ...
  def post(context)
    if context[:failed_files].any?
      ...
      polyphemus_client.update_run(@project_name, @run_id, @workflow_config_id, @workflow_version, {
        state: {
          failed_files: context[:failed_files].size,
