  ...
  describe Polyphemus::MagmaRecordScriptEtl::Scripts::IpiPatientFlowjo do
    xit 'runs without exception' do
      VCR.use_cassette('ipi+patient+flowjo.e2e') do
        magma_client = Etna::Clients::Magma.new(host: 'https://magma.ucsf.edu', token: ENV['TOKEN'] || 'test-token')
        metis_client = Etna::Clients::Metis.new(host: 'https://metis.ucsf.edu', token: ENV['TOKEN'] || 'test-token')
