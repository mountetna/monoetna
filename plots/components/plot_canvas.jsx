...
import {seriesLegend} from '../plot_config';
...
export default class PlotCanvas extends React.Component {
  ...
  render() {
    ...
    let colors = plot_series.map(
      ({name, variables: {color}}) => color || defaultColor(name)
    );

    let labels = plot_series.map((series, i) => {
    ...
    let svg_dimensions = {
      width: Math.max(Math.min(layout.width || parent_width, 1600), 300),
      height: layout.height
    };

    let xScale = this.scale(xdomain, [
      margin.left,
      svg_dimensions.width - margin.right
    ]);

    let yScale = this.scale(ydomain, [
      svg_dimensions.height - margin.bottom,
      margin.top
    ]);
