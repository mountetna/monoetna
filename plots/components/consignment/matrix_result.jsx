...
class MatrixResult extends React.Component {
  downloadMatrix() {
    ...
    TSV.downloadTSV(matrix_map, ['row_names'].concat(matrix.col_names), name);
  }
  ...
  render() {
    ...
    let data = matrix.map('row', (row, index, row_name) => ({
      row_name,
      ...row.reduce((row_data, value, col_index) => {
        row_data[matrix.col_names[col_index]] = value;
        return row_data;
      }, {})
    }));
