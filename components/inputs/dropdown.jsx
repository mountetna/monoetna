...
const offScreen = (bounds) => {
  return (
    bounds.top + 215 > document.documentElement.clientHeight + window.scrollY
  );
};
        ...
        render={(bounds) => (
          <div className={`dd-container ${className}`}>
            <div className='dd-header' onClick={this.toggleList}>
              ...
              <i
                className={list_open ? 'fa fa-angle-up' : 'fa fa-angle-down'}
              ></i>
            </div>
            {list_open && (
              <ul
                className={
                  'dd-list ' +
                  (offScreen(bounds) ? 'dd-list-up' : 'dd-list-down')
                }
              >
                {list.map((item, index) => (
                  <li
