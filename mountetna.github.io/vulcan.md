...
Some of the most basic, yet likely to be most used workflows of the Vulcan arsenal, these workflows consist of just a few steps where the first is a data query piece (which can be filled in via a [Timur-Query](https://mountetna.github.io/timur.html#query)) followed by a UI which determines how to make the given plot.  

#### The Setup GUI

A single system serves the graphical user interface (GUI) for mapping data to axes, colors, etc and allowing users to make tweaks to how their plot will show up. Thus, although there are some differences in the exact set of inputs that apply to each plot type, when a given input does show up, it will always work the same way!

'Main' inputs, which the user must fill in in order for a plot to be produced, are always shown within the GUI whereas 'Advanced' inputs 

**"make"** options: Often you will see string fields or other inputs filled in with the default string 'make'.  In these cases leaving that input as is with let our system fill in that input with an intuitive default.  These inputs do not need to be adjusted, but they can be given alternative options.
...
| Color Data | This dropdowns present column names of the incoming data for the user to select which column to use for coloring points. It also injects an option of 'make', the default, which translates to 'all one color' for scatter plots or 'the X-Axis Data selection' for Y Plots |
...
| Data Representations | The multi-select ui for this input allows users to selection which visual representations of their data to have in the plot: either violin plots, box plots, or both. |
...
| Follow selected render ordering when color is continuous? | When 'Color Data' is given a numeric data column, this checkbox input controls whether to follow the 'Point render order' selection for ordering the rendering of these points. |
| Point Size | This slider adjusts how large data points will be in scatter plots |
| Adjust scaling of the X-Axis?, Adjust scaling of the Y-Axis? | These dropdowns allow the user to change the scaling of the x and/or y axes from the default of 'as is' / linear, into being log10 scaled with two options for doing so: 'log10' simply converts scaling to log-scale; 'log10(val+1)' additionally adds 1 to all values and thus allows data with zeros (`log(0)` translates essentially to negative infinity!) to be successfully plotted in log scale. |
