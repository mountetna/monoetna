...
export const retrieveFiles =
  ...
  (dispatch) =>
    postRetrieveFiles(
      CONFIG.project_name,
      bucket_name || 'files',
      folder_name == undefined ? '' : folder_name
    )
      .then(({files, folders}) => {
...
export const listFilesRecursive =
  ...
  (dispatch) => {
    ...
    return postRetrieveFiles(
      CONFIG.project_name,
      bucket_name,
      folder_name
    ).then(({files, folders}) =>
      Promise.all(
        folders.map(({folder_name: subFolder}) =>
          listFilesRecursive({
...
export const downloadFilesZip =
  ({folder_name = '', files = [], bucket_name = ''}) =>
  (dispatch) => {
    async function addFiles(files, writer) {
      for (const file of files) {
        const { download_url, file_path } = file;
        const httpReader = new HttpReader(download_url);
        await writer.add(file_path, httpReader);
      }
    }

    // a stable identifier to share to cache some downzip work when re-clicked.
    // const downloadId = Array.from(
    //   files.map(({file_hash}) => file_hash).join('.')
    ...
    await addFiles(files, zipWriter);
    ...
    anchorElement.download = `${zipName}.zip`;
    anchorElement.click();
...
export const removeFile =
  ...
  (dispatch) => {
    ...
    deleteFile(CONFIG.project_name, bucket_name, file.file_path)
...
export const protectFile =
  ({file, bucket_name}) =>
  (dispatch) => {
    postProtectFile(CONFIG.project_name, bucket_name, file.file_path)
...
export const unprotectFile =
  ({file, bucket_name}) =>
  (dispatch) => {
    if (!confirm(`Are you sure you want to unprotect ${file.file_path}?`))
      return;

    postUnprotectFile(CONFIG.project_name, bucket_name, file.file_path)
      .then(({files}) => dispatch(addFiles(files)))
      .catch(
        errorMessage(
          dispatch,
...
export const renameFile =
  ({file, new_file_path, bucket_name, current_folder, new_bucket_name}) =>
  (dispatch) => {
    postRenameFile(
      CONFIG.project_name,
      bucket_name,
      ...
      .then(({files}) => {
        ...
        dispatch(
          addFiles(
            selectFilesInCurrentFolder({files, current_folder, bucket_name})
          )
        );
      })
      .catch(
...
export const touchFile =
  ({bucket_name, file}) =>
  (dispatch) => {
    getTouchFile(CONFIG.project_name, bucket_name, file.file_path)
      .then(({files}) => {
        dispatch(removeFiles([file]));
        dispatch(addFiles(files));
      })
