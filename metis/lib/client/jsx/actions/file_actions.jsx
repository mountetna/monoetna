...
import DownZip from 'downzip/dist/main';
  ...
  (dispatch) => {
    ...
    return downZip
      .downzip(
        downloadId,
        zipName,
        files.map(({size, download_url, file_path}) => ({
          name: file_path,
      ...
      .then((downloadUrl) => {
        if (!downloadUrl) {
          console.error(
            'Could not complete download, your browser may not support Service Workers or the service worker could not be installed.'
          );
          return;
        }
