...
export const postRetrieveFiles = (project_name, bucket_name, folder_name) =>
  json_get(
    `${window.location.origin}/${project_name}/list/${bucket_name}/${folder_name}`
  );

export const deleteFile = (project_name, bucket_name, file_name) =>
  json_delete(
    `${window.location.origin}/${project_name}/file/remove/${bucket_name}/${file_name}`
  );

export const postProtectFile = (project_name, bucket_name, file_name) =>
  json_post(
    `${window.location.origin}/${project_name}/file/protect/${bucket_name}/${file_name}`
  );

export const postUnprotectFile = (project_name, bucket_name, file_name) =>
  json_post(
    `${window.location.origin}/${project_name}/file/unprotect/${bucket_name}/${file_name}`
  );
...
export const postRenameFile = (
  ...
  return json_post(
    `${window.location.origin}/${project_name}/file/rename/${bucket_name}/${file_name}`,
    payload
  );
};
...
export const getTouchFile = (project_name, bucket_name, file_name) =>
  json_get(
    `${window.location.origin}/${project_name}/file/touch/${bucket_name}/${file_name}`
  );
...
export const postCopyFiles = (project_name, revisions) => {
  ...
  return json_post(
    `${window.location.origin}/${project_name}/files/copy`,
    payload
  );
};
