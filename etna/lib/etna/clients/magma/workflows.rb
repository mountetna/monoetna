  ...
  module Clients
    class Magma
      ...
      class ModelSynchronizationWorkflow < Struct.new(:target_client, :source_models, :target_project, keyword_init: true)
        ...
        # Potentially cyclical, protected aginst re-entry via the seen cache.
        ...
        def ensure_model_tree(model_name, seen = Set.new)
          ...
          attributes.attribute_keys.each do |attribute_name|
            ...
            ensure_model_tree(attribute.link_model_name, seen) unless attribute.link_model_name.nil?
            ensure_model_attribute(model_name, attribute_name)
          end
        end
        ...
        def ensure_model(model_name)
          ...
          parents = template.attributes.attribute_keys.map { |a| template.attributes.attribute(a) }.select { |a| a.attribute_type == AttributeType::PARENT }
