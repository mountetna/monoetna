...
class Magby(object):
    ...
    def retrieve(self,
        ...
        :param filterBy: str. Filter to pass to Magma query. See magma docs
        ...
        payload = self._constructPayload(projectName, modelName, recordNames, attributeNames,
                                         formatBy=typeSelection[0], filterBy=filterBy)
    ...
    def _walkMatrix(response: Dict, modelName: str, attributeName: str):
        observations = {x: response['models'][modelName]['documents'][x][attributeName] for x in
                        response['models'][modelName]['documents']}
