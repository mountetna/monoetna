...
import {empty, validDomain} from '../../utils/plot';
...
export const categoryGroups = (category, value, label, groupFunc) => {
  ...
  return category_names.map((category_name, index) => {
    let indexes = category
      .which((c) => c == category_name)
      .filter((i) => i != null);
    let category_values = value(indexes);
    let category_labels = label ? label(indexes) : [];

    let vl = category_values
      .map((_, i) =>
        category_values[i] != null
          ? [category_values[i], category_labels[i]]
          : null
      )
      .filter((_) => _)
      .sort((a, b) => a[0] - b[0]);
...
const SeriesComponent = ({
  ...
  let width = Math.max(
    4,
    xScale.bandwidth() / count -
      (gap_size * (count - 1) + 2 * gutter_size) / count
  );
...
export default class CategoryPlot extends Component {
  ...
  render() {
    ...
    let categories = flatten(
      plot_series.map((s) => s.variables.category.values)
    );
    ...
    return (
      <PlotCanvas
        ...
        onHighlight={(new_label) =>
          this.setState({highlighted_label: new_label})
        }
