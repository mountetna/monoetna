...
const Dots = (category_name, values, labels) => ({
  category_name,
  values,
  labels
});
...
const Dot = ({
  ...
  let dot = (
    <g>
      ...
      />
      {label && label == highlighted_label && (
        <text
          textAnchor={x > xmedian ? 'end' : 'start'}
          x={x + (x > xmedian ? -4 : 4)}
          y={y}
        >
          {label}
...
class Swarms extends Component {
  ...
  calculateSwarmPoints() {
    ...
    let {
      variables: {category, value, label}
    } = series;
    ...
    this.worker = createWorker(
      require.resolve('./../../workers/swarm_plot.js'),
    ...
    Object.values(groups).forEach(({category_name, values}) => {
  ...
  render() {
    ...
    let {
      variables: {category, value, label, model}
    } = series;
    ...
    return (
      <g className='swarm-series' key='swarms'>
        {swarms}
      </g>
    );
  }
