...
const isDate = (value) => typeof value === 'string' && value.match(ISO_FORMAT);

const isObject = (value) => value != null && typeof value == 'object';

const isMatrix = (value) =>
  'matrix' in value &&
  'rows' in value.matrix &&
  !(value.matrix instanceof Matrix);
...
const reviver = (key, value) => {
  if (isDate(value)) return new Date(value);
    ...
    if (isMatrix(value)) return new Matrix(value.matrix);
    else if (isVector(value)) return new Vector(value.vector);
    else return value;
  } else return value;
};
...
export default class Consignment {
  constructor(consignment) {
    let parsed = JSON.parse(JSON.stringify(consignment), reviver);
