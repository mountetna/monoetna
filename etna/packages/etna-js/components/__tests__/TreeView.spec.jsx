...
class TreeViewTest {
  ...
  findInput(path, node) {
    return this.mounted
      ...
      .findWhere(
        (i) =>
          i.props()['data-path'].join(',') === path.join(',') &&
  ...
  get selected() {
    return this.mounted
      .find('input[type="checkbox"]')
      .getElements()
      .filter((e) => e.props.checked)
      .map((e) => e.props['data-path'].concat([e.props['data-node']]));
  }
  ...
  describe('with some options', () => {
      ...
      test.options = [
        [
          ...
          [
            ...
            [
              'Inner 2',
              [['Inner Inner 1'], ['Inner Inner 2'], ['Inner Inner 3']]
            ],
            ['Inner 3']
          ]
        ],
    ...
    describe('selecting a top level', () => {
      ...
      it('disables recursively child nodes', () => {
        expect(test.selected).toEqual([['Top Level 2']]);

        expect(test.lastSelectionsByCallback).toEqual(['Top Level 2']);
      });

      describe('selecting each leaf', () => {
        it('continually enables those leaves and eventually the parents again, too', () => {
          test
            .findInput(['Top Level 1', 'Inner 2'], 'Inner Inner 1')
            .simulate('change');
          expect(test.selected).toEqual([
            ['Top Level 1', 'Inner 2', 'Inner Inner 1'],
            ['Top Level 2']
          ...
          test
            .findInput(['Top Level 1', 'Inner 2'], 'Inner Inner 2')
            .simulate('change');
          expect(test.selected).toEqual([
            ['Top Level 1', 'Inner 2', 'Inner Inner 1'],
            ['Top Level 1', 'Inner 2', 'Inner Inner 2'],
          ...
          test
            .findInput(['Top Level 1', 'Inner 2'], 'Inner Inner 3')
            .simulate('change');
          expect(test.selected).toEqual([
            ['Top Level 1'],
            ['Top Level 1', 'Inner 1'],
