...
When not provided explicitly, as is the other method, the user will be prompted to provide their token via the interactive console. It is recommended that you store the output of your call to `magmaRset()` as a variable and then to provide this variable within each subsequent call to a magmaR function, as below.
