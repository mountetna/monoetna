...
prod <- magmaRset(token = Sys.getenv("TOKEN"), url = Sys.getenv("URL"))
```
...
Users should not typically need to make use of this function, as when the desired data is a matrix, `retrieveMatrix()` is recommended instead. More details on that function are further below.
...
Details: The output format for the `retrieveJSON()` function is a nested list conversion of the nested hash json output that is directly given by a call to magma/retrieve with the json equivalent of `format = "json"`.
...
For more details, see the function documentation with `?retrieve` or `?retrieveJSON`. And for even more detail, such as how to format filter inputs, refer to https://mountetna.github.io/magma.html#retrieve.
