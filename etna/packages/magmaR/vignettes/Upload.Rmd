...
**Common features to all magmaR update functions:**

- A summary of updates to be made will always be presented, prior to transferring the update payload to magma. Currently, the summary describes record IDs for each mode that would be created or updated, but that summary may grow in the future.
- Then, the user will always be prompted to check that the summary matches their intended updates, unless the input `auto.proceed` is set to FALSE.
...
Utility: Useful for smaller updates, carefully scripted automation, or any updates that cannot be handled with the `updateFromDF()` and `updateMatrix()` wrappers. Its `revisions` structure can feel a bit clunky, but is ultimately very powerful.
...
## Important Consideration when adding NEW records
