...
2. Some input parsing.
3. Make a curl request that calls on a magma function to either send or receive desired data.
...
- `updateValues()` = a few-frills wrapper of **magma**'s sole data input function, **`/update`**.  It's quite flexible, but that flexibility owes largely to a rigid input structure, so we also provide a few wrapper functions to make some common upload needs easier.
...
Jumping in, the first thing to know is that **all `update` functions will**: 
...
- Lastly, the success/failure status of the post to `magma/update` will be presented, including any error messages returned.

For example, you might see a summary like this:
...
**It is highly recommended that these summaries be checked carefully for accuracy before proceeding as `update`s have the power to overwrite magmaR data!**

There is currently no history in magma, so updates cannot be rolled back.

Also, contrary to the "update" portion of the function names, these functions can add totally new data to magma records.
Once created, it is not easy to remove records with an incorrect ID.
**So if you get a WARNING message like in the example above, please heed the warning and double-check your summary!**

That said, to continue, simply `y`, `yes`, or the like, and hit `enter` to proceed.
...
The "biospecimen" and "rna_seq" models will be our perticular targets, and have attributes...
...
Internally, the function performs some necessary validations, adjusts the matrix into the proper `revisions`-input format, then passes it along to `updateValues()` where all the common functionality is housed.
...
- Column Names (or the first row when providing a file) = the `recordNames` that should be updated
- Row Names (or the first column when providing a file) = among the allowed 'options' for the target attribute. (Ex: gene ids for rna_Seq data)
- Column Data = the values for each record. 

When `matrix` points to a file, the optional `separator` input can be use to indicate between csv (default assumption) versus tsv parsing.
...
This function wraps `updateValues()` to allow easier update of a set of attributes for of multiple records, via a simpler, rectangular, data input structure.

Internally, the function performs some necessary validations, adjusts the input into the proper `revisions`-input format, then passes it along to `updateValues()` where all the common functionality is housed.
...
A separate `df` input then takes in the target data. This can be given as data.frame directly, or a string representing the path to a csv or tsv file containing such data. In all cases, `df` must be formatted to have:
...
`data.frame` structure, or via a csv/tsv file encoding such data.

### updateValues()

`updateValues()` is the main workhorse function of `magmaR`'s data upload capabilities. It largely mimics `magma/update`, where each target record is indicated individually, giving very flexible control. The function relies on a rigid nested *list* structure, and is at the heart of all updates made via magmaR.

Utility: Useful for smaller updates, any updates that cannot be handled with the `updateFromDF()` and `updateMatrix()` wrappers, and carefully scripted automation scripts.

Its `revisions` input structure can feel a bit clunky, but is ultimately very powerful.

The function has 3 main inputs, `project` and `revisions`:

- `target`, an integral bit of any call to `magmaR`. It describes how to connect with `magma`. See the dedicated section of the download-focused vignette for more details.
...
As mentioned previously, it is highly recommended that these outputs be checked carefully for accuracy before proceeding.
