...
Internally, the function performs some necessary validations, adjusts the matrix into the rigid `revisions`-input format of `updateValues()`, then passes its payload along to `updateValues()` where all the common update functionality is housed.
...
Utility: When aiming to update the same set of attributes for many different records, which is probably the most common case when wanting to upload data, use this function. For matrix attributes, use `updateMatrix()` instead.
...
When `df` points to a file, the optional `separator` input can be use to indicate between csv (default assumption) versus tsv parsing.

**Example:**

To update the  of our "rna_seq" model from either a csv, a tsv, or directly from a matrix, we could use the code below:
...
1. The left-most column of df holds record identifiers.
2. Column 2 onwards are data columns which hold intended data for attributes of each target record.
3. Column names 2 onwards of df are the target attribute-names.

*One final note:* The identifier column's name does not actually matter. 'tube_name' is the name of the identifier attribute of this "rna_seq" model, but we could have named the column anything. This is an intended feature as it allows users to provide a data column with the identifier attribute's name in order to adjust records' identifiers!
...
Utility: Useful for smaller updates, for any updates that cannot be handled with the `updateFromDF()` and `updateMatrix()` wrappers, and for carefully scripted automation scripts.

#### Necessary inputs:

In addition to the normal `target` and `projectName` inputs, the `revisions` is the primary input of `updateValues()`:
