...
2. Make a curl request to magma to obtain desired data.
3. Perform some level of, typically minimal, restructuring of the data.
5. Return the output.

### Data Restructuring Details

Often, a **tsv** will be obtained from magma, which magmaR will read in as a **dataframe**.

Othere times, magma will return data as a **JSON**. Unfortunately, JSON object don't play well in R so in these cases, the data will be converted to a **nested list**.
...
- `retrieveMatrix()`: downloading an entire matrix in the necessary JSON format ends up being very inefficient, so this function obtains data for chunks of records at a time, then converts to a matrix and appends those chunks together `recordNames` as column names, and row names pulled from the project template.
