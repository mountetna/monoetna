...
export const inputPath = async (
  name: string,
  inputsEnv: {[key: string]: any} = process.env,
  inputsDir: string | null = null
): Promise<string> => {
  let path: string;
...
export const outputPath = async (
  name: string,
  outputsEnv: {[key: string]: any} = process.env,
  outputsDir: string | null = null
): Promise<string> => {
  let path: string;
    ...
    return _access(path, _fs.constants.F_OK)
      ...
      .catch((err) => {
        return Promise.reject(
          `Output key ${name} does not exist for this cell`
        );
      });
  } else {
