      ...
      Array(numsegs+1).fill().map(
        (_,i) => {
          ...
          let jitter = 1 * 
            ...
            // basic sin
            sin(counter*pulse + freq * PI * i/numsegs)*amp +
            // random jitter
            JITTERS[(counter+i)%300]*3-1.5;
...
const Halo = ({radius}) => {
  ...
  return <div className='halo'>
    <svg>
      ...
      {
        segments.map(
          (seg,i) => <path key={i} d={ seg.path() } style={{
            stroke: seg.color, strokeWidth: seg.width,
