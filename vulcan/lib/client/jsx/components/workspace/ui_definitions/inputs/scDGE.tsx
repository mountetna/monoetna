...
const method_labels: DataEnvelope<string> = {
  '1 btwn-all-de-groups: Between All labels of one metadata':
    'btwn-all-de-groups',
  '2 btwn-sets: Between particular labels': 'btwn-sets',
  '3 btwn-sets-multiple-groups: Between particular labels, in multiple cell groups':
      ...
      if (Object.keys(opts).length > 0 && vals['de_meta'] != null) {
        if (Object.keys(vals).includes('de_group_1')) {
          value_select_1 = MultiselectPiece(
            ...
            changeFxn,
            vals['de_group_1'],
            'Labels to include in Set-1',
            opts[vals['de_meta']] as string[]
          );
        }
      ...
      if (Object.keys(opts).length > 0 && vals['group_meta'] != null) {
        if (Object.keys(vals).includes('group_use')) {
          value_select = MultiselectPiece(
            ...
            vals['group_use'],
            'Labels to target',
            opts[vals['group_meta']] as string[]
          );
        }
      }
...
export default function DiffExpSC({
  ...
  const warn = (
    <div>
      <p></p>
      IMPORTANT NOTE: The scanpy DE algorithm cannot correct for batch effects.
      We are currently investigating alternatives to add such functionality.
      <p></p>
    </div>
  );
  ...
  if (value) {
    if (value['subset']) {
      SubsetComps = subsetDataFramePiece(
        ...
        updateValue,
        value['subset'],
        'Step 2: Subset by features to focus on certain cells? ',
        {...allData},
        false,
        'secondary'
      );
