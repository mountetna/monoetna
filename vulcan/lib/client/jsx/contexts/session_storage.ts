...
const localStorageKey = ({figure_id,project_name,workflow_name}) => (
  figure_id
...
export function useLocalSessionStorage(state: VulcanState,
  ...
  const getLocalSession = useCallback((workflow_name: string, figure_id: int, project_name: string) => {
    ...
    try {
      ...
      if (parsedSession.project_name !== project_name) return Promise.resolve(null);
