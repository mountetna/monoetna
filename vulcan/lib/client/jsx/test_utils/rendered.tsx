...
export function findNode(
  ...
  predicate: (e: ReactTestInstance) => boolean,
  idx: number
) {
...
export function includesClassNamePredicate(className: string) {
  return (node: ReactTestInstance) =>
    node.props.className?.match(new RegExp(`(?:\\s+|^)${className}(?:\\s+|$)`));
}

export function matchesTypePredicate(
  element: string | ((props: any) => ReactElement)
) {
