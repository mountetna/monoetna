...
class WorkflowsController < Vulcan::Controller
  ...
  def fetch_for_project
    success_json({
      'workflows': workflows_for(@params[:project_name])
    })
  end
  ...
  def workflows_by_tag(workflows, tag)
    workflows.select do |workflow|
      workflow[:tags]&.include?(tag)
    end
  end

  def all_workflows
    Dir.glob(Vulcan.instance.config(:workflows_folder) + "/*.{cwl,yaml,yml}").sort_by { |f| ::File.basename(f) }.map do |path|
